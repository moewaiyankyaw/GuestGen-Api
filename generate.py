...
# === Fixed Credentials ===
DEFAULT_NAME_PREFIX = "Hawk"
DEFAULT_PASSWORD_PREFIX = "Hawk123"

# === Watermark ===
WATERMARK = "Guest Gen API By - @M69431 (TG)"
...
class FreeFireAccountGenerator:
    def create_acc(self, region, is_ghost=False):
        password = generate_custom_password(DEFAULT_PASSWORD_PREFIX)
        ...
        try:
            ...
            if resp.status_code == 200 and 'uid' in resp.json():
                uid = resp.json()['uid']
                time.sleep(0.5)
                return self.token(uid, password, region, is_ghost)
        except Exception as e:
            print(f"Create account error: {e}")
        return None

    def token(self, uid, password, region, is_ghost):
        body = {
            "uid": uid,
            "password": password,
            "response_type": "token",
        ...
        try:
            ...
            if resp.status_code == 200 and 'open_id' in resp.json():
                ...
                return self.Major_Regsiter(access_token, open_id, field, uid, password, region, is_ghost)
        except Exception as e:
    ...
    def Major_Regsiter(self, access_token, open_id, field, uid, password, region, is_ghost):
        name = generate_random_name(DEFAULT_NAME_PREFIX)
...
def generate_accounts():
    ...
    # === Get allowed params: count and region ===
    count_str = request.args.get('count', '1')
    region = request.args.get('region', 'BR').upper()
    ghost_str = request.args.get('ghost', 'false').lower()
    is_ghost = ghost_str == 'true'
    ...
    try:
        ...
        return make_cors_response({
            "error": "Invalid count. Must be number 1-10."
        }), 400

    # Validate region
    valid_regions = list(REGION_LANG.keys())
    if region not in valid_regions and not is_ghost:
        return make_cors_response({
            "error": f"Invalid region. Valid: {', '.join(valid_regions)}"
        }), 400
    ...
    for i in range(count):
        account = generator.create_acc(region, is_ghost=is_ghost)
        if account:
            is_rare, score, _ = check_account_rarity({"account_id": account["id"]})
            results.append({
                "uid": account["uid"],
...
def health():
    resp = jsonify({"status": "ok", "credit": WATERMARK})
    resp.headers.add("Access-Control-Allow-Origin", "*")
    return resp
